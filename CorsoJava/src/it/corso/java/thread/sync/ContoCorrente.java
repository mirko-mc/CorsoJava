...
public class ContoCorrente {
	...
	public double getSaldo() {
		...
		try {
			File fin = new File(new File(".").getCanonicalPath() + File.separator + "\\EXT\\saldo.txt");
	...
	public synchronized void prelievo(double somma) throws Exception {
		...
		try {
			...
			if (nuovoSaldo > 0) {
				fw = new FileWriter(new File(".").getCanonicalPath() + File.separator + "\\EXT\\saldo.txt");
				bw = new BufferedWriter(fw);
				bw.write(nuovoSaldo+"");
				} else throw new Exception("saldo insufficiente");
			} catch (IOException e) {
				e.printStackTrace();
				} finally {
					try {
